...
describe('Hero CTA styles live in _hero.scss', () => {
  it('captures base, mobile, and desktop rules', () => {
    ...
    // Base row spacing and alignment: CTAs sit left-aligned inside the copy
    // column rather than spread across it (6f820cd, 9d5d04f).
    expect(contents).toMatch(
      /\.HeroCTARow\s*\{[\s\S]*?margin-top:\s*clamp\(26px,\s*4\.5vw,\s*48px\);[\s\S]*?justify-content:\s*flex-start;/,
    );
    expect(contents).toMatch(
    ...
    expect(contents).toMatch(
      /@include\s+down\(\$bp-md\)[\s\S]*?\.HeroCTA[\s\S]*?@include\s+type-scale\(14px,\s*4vw,\s*18px\);/,
    );
    expect(contents).toMatch(
